...
import type { DomainEntry, GlobalOAuthAccount, IdentityProfile } from "../../storage/schema";
...
async function syncLinkedApps() {
  ...
  appCards.forEach(card => {
    ...
    const domainMatches = cardText.match(/[a-zA-Z0-9-]+\.[a-zA-Z]{2,}(?:\.[a-zA-Z]{2,})?/g);
    if (domainMatches) {
      domainMatches.forEach(d => {
        if (!d.includes('google') && !d.includes('gstatic') && d.includes('.') && d.length > 4) {
...
async function saveToVault(emails: string[], domains: string[]) {
  try {
    ...
    let vault = (await extensionStorage.get<DomainEntry[]>("saved_accounts")) || [];
    let modifiedRegistry = false;
    let modifiedVault = false;

    for (const email of emails) {
      const existsInRegistry = registry.find(r => r.provider === "google.com" && r.identity === email);
      ...
      for (const domain of domains) {
        ...
        if (entryIdx >= 0) {
          const hasIt = vault[entryIdx].accounts.some(
            a => a.login_method.type === 'oauth' && a.login_method.provider === 'google.com' && a.identities[0] === email
          );
          if (!hasIt) {
    ...
    if (modifiedRegistry) await extensionStorage.set("oauth_registry", registry);
    if (modifiedVault) {
      await extensionStorage.set("saved_accounts", vault);
      log.info(`Synced ${domains.length} apps × ${emails.length} emails to Vault!`);
    }
  } catch (e) {
